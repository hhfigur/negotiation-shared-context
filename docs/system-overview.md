...
└─────────┼─────────────────┼────────────────────┼────────────────────────────┘
          │                 │                    │
          │ Backend REST     │ Supabase JS SDK    │ Supabase Auth
          ▼                 ▼                    ▼
┌──────────────────┐  ┌─────────────────────────────────────────────────────┐
│ Render.com        │  │ Supabase (PostgreSQL + Auth + Edge Functions)       │
│ negotiationcoach- │  │                                                     │
│ backend           │  │  ┌──────────────────┐  ┌────────────────────────┐  │
...
| Express API | negotiationcoach-backend | TypeScript | Render.com | Analysis, market data, LLM orchestration |
...
**Note (Observed):** Two parallel backend runtimes exist — the Express Backend (Render.com) and a Supabase Edge Function (`negotiate`) that independently implement ZOPA, Nash, Monte Carlo, and strategy scoring with a different input schema. These are NOT in sync.
...
|---------|---------|------------|
| Anthropic Claude API | LLM inference (haiku/sonnet/opus) | Express backend only |
| Supabase Auth | JWT issuance, email verification, password reset | Frontend + Express backend middleware |
| Supabase PostgreSQL | Primary datastore | Frontend (direct) + Express backend |
...
```
User fills form → NegotiationCanvas.tsx → Backend POST /api/analyze-full
...
### 5.3 Plan Generation
```
AnalysisContext → Backend POST /api/plan
...
useSessionManager.ts → Supabase SDK direct writes
  ...
  [No Backend API intermediary — frontend writes DB directly]
```
...
| Source | Tier Values |
|--------|-------------|
| Express backend (src/types/index.ts) | `free \| privat \| kmu \| profi` |
...
|----|------|----------|----------------|
| SYS-01 | Dual Layer 1 implementations (Express backend src/ vs Edge Function negotiate/) with divergent type schemas and no sync mechanism | Critical | Observed |
| SYS-02 | Team admin authorization is frontend-only (no RLS enforcement verified) | High | Observed |
| SYS-03 | backend authMiddleware never rejects — no 401 issued in dev mode | High | Observed |
...
| SYS-05 | CORS wildcard header overrides allowlist in Express backend | Medium | Observed |

...
|-------|-----------|-----------|-----------------|------------|-----------------|
| `negotiation_sessions` | Railway (`GET /api/sessions/:id`), Frontend SDK (reads) | Railway (`POST /api/sessions`, `PATCH /api/sessions/:id`, `POST /api/analyze`, `POST /api/enrich`) + Frontend SDK (violation — Phase B pending) | SERVICE_ROLE_KEY (Railway); anon key (frontend violation) | No — code-enforced via `assertSessionOwner()` | Railway backend |
| `session_messages` | Frontend SDK (`useSessionManager.ts` — direct select, last 50) | Railway (`POST /api/sessions/:id/messages`) + Frontend SDK (violation — Phase B pending) | SERVICE_ROLE_KEY (Railway); anon key (frontend violation) | No — RLS migration pending (RFB-030) | Railway API (`sessionRoutes.ts`) |
...
**RLS Status:** No policies exist. Ownership enforced at application level via `assertSessionOwner()` and `.eq('user_id', req.user.id)`. SERVICE_ROLE_KEY bypasses RLS for Railway writes. Migration pending: RFB-030.
...
| Operation | Caller | Method | Auth | Business Rule |
|-----------|--------|--------|------|---------------|
| INSERT | Railway `POST /api/sessions/:id/messages` | `supabase.from('session_messages').insert()` | SERVICE_ROLE_KEY | Count-before-insert enforces 50-message limit (non-atomic — DB constraint pending RFB-004-C) |
...
| INSERT (violation) | Frontend `useSessionManager.ts` | `supabase.from('session_messages').insert()` | anon key | Fire-and-forget with retry — no 50-message limit enforced client-side — **Phase B pending** |

**RLS Status:** No policies exist. No migration file found for this table (origin untracked). RLS migration pending: RFB-030.

**Note:** `session_messages` origin is untracked — no `CREATE TABLE` migration exists in either repo. The table exists in production but its schema is inferred from usage patterns.
...
|-------|------|-------------------|-------|
| `negotiation_sessions` | High — if anon key used directly, cross-user reads possible | RFB-030 | Railway uses SERVICE_ROLE_KEY; code-level guard present via `assertSessionOwner()` |
| `session_messages` | High — no ownership check at DB layer | RFB-030 | Phase B frontend migration blocked until RLS exists |
...
| VG-ID | Question | Risk |
|-------|----------|------|
| VG-02 | Does RLS on `negotiation_sessions` exist? If applied, does it conflict with Railway's code-level ownership check? | High |
| VG-DB-01 | Does `session_messages` have a `CREATE TABLE` migration in either repo? What is the definitive schema? | Medium |
